basedir = "D:\\!CS PAPER\\cs-2014-experiments\\biplane\\SNR+density";
methods = newArray("CS-MLE");
bkg = 70.0;
tol_export_dist = 200;
	...
	for(intensity = 2500; intensity <= 2500; intensity += 1000) {
		...
		for(tol = 50; tol <= 1000; tol += 50) {
			...
			if(tol == tol_export_dist) {
				wait(200);
				run("Export results", "z=false filepath=["+basedir+"\\density="+density+"\\results\\"+method+"+I="+intensity+"+bkg="+bkg+"_tol="+tol+".csv] fileformat=[CSV (comma separated)] sigma_y1=false sigma_y2=false id=false sigma_x1=false sigma_x2=false frame=true gt_dist_xy=true gt_dist_z=true gt_dist_xyz=true gt_id=true saveprotocol=false offset=false i=false y=false x=false sigma1=false sigma2=false intensity=false bkgstd=false chi2=false uncertainty=false z_rel=false");
			}
		}
